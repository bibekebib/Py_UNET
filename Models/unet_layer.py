...
class DownStage(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2),
            ThreeConvBlock(in_channels, out_channels)
        )
...
class UpStage(nn.Module):
    def __init__(self, in_channels, out_channels, bilinear=True):
        ...
        if bilinear:
            ...
            self.conv = ThreeConvBlock(in_channels + out_channels, out_channels)
        else:
    ...
    def forward(self, x1, x2):
        ...
        x = torch.cat([x2, x1], dim=1)  # Concatenating along the channel dimension (dim=1)
